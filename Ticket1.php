...
JAVASCRIPT;
    ...
    public static function getListForItemRestrict(CommonDBTM $item)
    {
        ...
        // Check if ticket is already taken (i.e., stolen)
        if (isset($item->fields['user_id_taken']) && $item->fields['user_id_taken'] > 0) {
            // The ticket has been stolen, so restrict it from being stolen again
            $restrict['glpi_tickets.id'] = ['neq' => $item->getID()];  // No one can steal this ticket
        }
        ...
        switch (get_class($item)) {
            ...
            default:
                ...
                // You can only see your tickets or the ones you are assigned to
                if (!Session::haveRight(self::$rightname, self::READALL)) {
                    $or = [
                        'glpi_tickets.users_id_recipient' => Session::getLoginUserID(),
                        [
