...
function plugin_lock_install() {
   ...
   // Check if the special LOCK profile exists in glpi_profiles table
   // If not, add it
   $query = "SELECT id FROM glpi_profiles WHERE name LIKE 'Plugin Lock % Profile'";
   $result = $DB->query($query);
   if ($DB->numrows($result) == 0) {
       // Profile is not existing, must create an empty one
       $prof = new Profile();
       $prof->add([
       ...
       if ($profile_id > -1) {
           ...
           $profRights->updateProfileRights($profile_id, [
               'computer' => 1,
               'monitor' => 1,
               'ticket' => 1024,
               'task' => 8193,
   ...
   CronTask::Register('PluginLockLock', 'unlock', DAY_TIMESTAMP, ['param' => 4, 'state' => CronTask::STATE_DISABLE, 'mode' => CronTask::MODE_EXTERNAL]);
